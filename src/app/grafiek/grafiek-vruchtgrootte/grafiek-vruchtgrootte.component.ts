...
@Component({
  selector: 'app-grafiek-vruchtgrootte',
  templateUrl: './grafiek-vruchtgrootte.component.html',
  styleUrls: ['./grafiek-vruchtgrootte.component.scss']
})
export class GrafiekVruchtgrootteComponent implements OnInit {
  ...
  ngOnInit() {
    this._databaseService.getAppelWaardesByArduino().subscribe(result => {
      console.log(result);
      this.lineChartData[0].data = result;
    });
    this._databaseService.getPeerWaardesByArduino().subscribe(result => {
      console.log(result);
